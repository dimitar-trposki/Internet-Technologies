...
namespace AlbumsMVC_Kolokviumska.Controllers
{
    public class StoresController : Controller
    {
        ...
        {
            StoreAlbums model = new StoreAlbums();
            model.StoreId = id;
            model.Albums = db.Albums.ToList();
            ViewBag.StoreName = db.Stores.Find(id).Name;
            return View(model);
        }

        [HttpPost]
        public ActionResult AddToStore(StoreAlbums model)
        {
            var store = db.Stores.Find(model.StoreId);
            var album = db.Albums.Find(model.SelectedAlbumId);
